/* This file will run with the CPU in parallel*/
#include <stdio.h>
#include <stdlib.h>
#include <omp.h>
...
void convolve_parallel(float **image, float **output, float **kernel, int width, int height, int kernel_size) {
    int offset = kernel_size / 2;

    // Apply kernel to each pixel in parallel
